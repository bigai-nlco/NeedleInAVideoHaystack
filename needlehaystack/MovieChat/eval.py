...
def process_video(input_video_filename, insert_needle, modality, insert_point, output_video_filename=None, tmp_dir='.tmp/'):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4', dir=tmp_dir) as temp_file:
        output_video_filename = temp_file.name
        # Write the result to the temporary file
        # final_clip.write_videofile(output_video_filename, codec="libx264", fps=main_video_clip.fps, logger=None)
...
# Example usage:
# # To add text
# output_video_path = process_video("test.mp4", "The secret word is 'needle'", "text", 5)
